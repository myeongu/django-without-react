...
def signup(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            signed_user.send_welcome_email() # FIXME: 비동기 or Celery 사용 추천
...
def user_follow(request, username):
    ...
    # request.user => follow_user을 팔로우하고자 함
    request.user.following_set.add(follow_user)
    follow_user.follower_set.add(request.user)

    messages.success(request, f"{follow_user}님을 팔로우했습니다.")
    redirect_url = request.META.get("HTTP_REFERER", "root") # META에 REFERER가 있으면 가져오고 아니면 'root'를 반환
...
def user_unfollow(request, username):
    ...
    redirect_url = request.META.get("HTTP_REFERER", "root") # META에 REFERER가 있으면 가져오고 아니면 'root'를 반환
    return redirect(redirect_url) # root는 url의 패턴네임이지만 redirect자체가 패턴네임으로 먼저 시도해보고 이후 url로 처리하므로 OK